...
+ (NSString *)deviceNetType{
    ...
    DM_Reachability *reach = [DM_Reachability reachabilityForInternetConnection];
...
+ (NSString *)deviceLANIp{
    ...
    if ([[self deviceNetType] isEqualToString:@"Wi-Fi"]) {
        NSString *wfKey = [NSString stringWithFormat:@"%@/%@",IOS_WIFI,IP_ADDR_IPv6];
        NSString *wfIp = [addressDict objectForKey:wfKey];
        return wfIp?wfIp:@"";
    }else{
        NSString *cellKey = [NSString stringWithFormat:@"%@/%@",IOS_CELLULAR,IP_ADDR_IPv6];
